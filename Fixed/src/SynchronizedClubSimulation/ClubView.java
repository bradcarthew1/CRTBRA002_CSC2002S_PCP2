...
public class ClubView extends JPanel implements Runnable {
		...
		private final int maxY; //maximum Y for the grid
		private final int maxX; //Maximum X for the grid

		ClubGrid grid; //shared grid
		
		ClubView(PeopleLocation[] customers, ClubGrid grid, int []exits) { //constructor
			this.patronLocations = customers;
			noPatrons = customers.length;
			this.grid = grid;
			this.exits = exits;
